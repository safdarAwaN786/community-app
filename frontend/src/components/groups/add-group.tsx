...
export default function AddGroupRoom({ setIsModal, isModal }: groupRoomT) {
  ...
  const joinGroup = async (index: number) => {
    if (adding === null) {
      ...
      try {
        // Join the group
        const joinedGroup = await joinGroupAPI(Cookies.get("userToken"), index.toString());
                    ...
                    <li className="py-3 sm:py-4" key={group._id}>
                      <div className="flex items-center">
                        ...
                        <button
                          ...
                          className={`inline-flex rounded-lg items-center ${adding === index ? "cursor-not-allowed opacity-50" : "hover:bg-[#615EF0]"
                            }`}
                        >
                          {adding === index ? (
                            <div
